...
const password = (query) => readlineSync.question(query, { hideEchoBack: true });
const question = (query) => readlineSync.question(query, { hideEchoBack: false });

const buildConnection = (user, pass, server) =>
    `mongodb://${user}:${pass}@${server}:27017/sample-database?tls=true&replicaSet=rs0&readPreference=secondaryPreferred&retryWrites=false`;

const defaultConnection = () =>
    buildConnection(
        process.env.DB_USER || 'adminuser',
        process.env.DB_PASS || 'Hanumanji10',
        process.env.DB_SERVER || 'docdb-2025-01-02-17-12-50.c7ooww4i43ft.ap-southeast-2.docdb.amazonaws.com'
    );

const readConnectionFromUser = () => {
    console.log('\nUsing default connection details.');
    if (question('Do you want to override any details (yes/no)? ').toLowerCase() === 'yes') {
        const user = question('Enter username (default: adminuser): ') || 'adminuser';
        const pass = password('Enter password (default: Hanumanji10): ') || 'Hanumanji10';
        const server = question(
            'Enter server (default: docdb-2025-01-02-17-12-50.c7ooww4i43ft.ap-southeast-2.docdb.amazonaws.com): '
...
app.get('/test', (req, res) => {
    res.json({
        ...
        version: '1.0.0',
        developer: { name: 'Your Name', email: 'your.email@example.com' },
    });
});
